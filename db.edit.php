...
$kairos = htmlentities($_GET['kairos']);
...
$mysqli = connect();
$stmt = $mysqli->prepare("UPDATE `day` SET `MemoryVerse` = ?, `Gratitude` = ?, `Prayer` = ?, `Intent` = ?,  `Kairos` = ?,  `Reflection` = ? WHERE `day`.`Today` = '$date' AND `day`.`username` = '$username'");
$stmt->bind_param("ssssss", $memverse, $gratitude, $prayer, $intent, $kairos, $reflection);
$stmt->execute();
        ...
        <section class="history">
            <div class="entry">
                ...
                <div class="entry-banner">
                    ...
                    <div class="entry-col">
                        <form action="edit.php"><input type="hidden" name="day" value="<?php echo $date ?>"><button type="submit" class="btn-journal">Edit</button></form>
                    </div>
                </div>
